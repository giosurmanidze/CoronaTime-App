            ...
            <div class="w-full">
                <div class="w-[90%] flex justify-between align-center m-auto h-[60px]">
                    ...
                    <div class="flex items-center sm:gap-[25px] xs:gap-3">
                        <div class="text-black flex items-center cursor-pointer">
                            <select id="language-select" class="cursor-pointer" onchange="window.location.href = this.value;">
                                <option  value="{{ route('landing-worldwide', ['language' => 'en']) }}"
                                    @if (app()->getLocale() === 'en') selected @endif>{{ __("English") }}</option>
                                <option value="{{ route('landing-worldwide', ['language' => 'ka']) }}"
                                    @if (app()->getLocale() === 'ka') selected @endif>{{ __("Georgian") }}</option>
                            </select>
                        </div>
                ...
                <div class="w-[90%] flex flex-col justify-center m-auto h-[120px] gap-5">
                    ...
                    <div class="flex gap-10 relative">
                        ...
                        <a href="#">{{ __("by_country") }}</a>
                        <div class="h-[1px] bg-[#F6F6F7] w-full absolute top-9"></div>
                    </div>
                </div>
            </div>
            <div class="grid md:grid-cols-3 xs:grid-cols-2 w-[90%] m-auto gap-3 mt-6">
                <x-info-card img="card-1.svg" bg_color="bg-[#2029F3]"
                    class="text-[#2029F3] md:col-span-1 xs:col-span-2"  title="new_cases"/>
                <x-info-card img="card-2.svg" bg_color="bg-[#0FBA68]"  title="Recovered" class="text-[#0FBA68] md:col-span-1" />
                <x-info-card img="card-3.svg" bg_color="bg-[#EAD621]"  title="Death" class="text-[#EAD621] md:col-span-1" />
            </div>
        </div>
    </x-slot>
</x-layout>
